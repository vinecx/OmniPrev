...
import PreventivasActions from '../../../../shared/@types/model/preventivas/preventivas.actions';
import {
  Button,
  ButtonText,
...
const Create = () => {
  ...
  const preventivaActions = new PreventivasActions();
  ...
  const submit = async () => {
    if (hasImagesToUpload && fileUploadStatus === 'IDLE') {
      ...
      const { error, errorMessage } = await preventivaActions.cadastrar(values);

      if (!error) {
        ToastAndroid.show(
          `Preventiva ${isEditScreen ? 'salvo' : 'cadastrado'} com sucesso`,
          ToastAndroid.SHORT,
        );
        goBack();
      } else {
